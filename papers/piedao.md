...
The models aim to mitigate a common condition where due to increased decentralization \(more number of token holders\) participation in voting decreases, making decision making less agile and prone to answer quickly to challenges.

### Stakeholders

**DOUGH Token Holders**: DOUGH is the most important asset for decision making. Any decision can be taken or revoked using DOUGH. DOUGH is a non-tradable asset, it functions within the PieDAO governance mechanism alone. Holders of DOUGH retain the possibility of taking decisions in the DAO and exit the DAO \(aka ragequit\) anytime.
...
A Committee member can charge a fee on rewards generated by delegated funds. Such fee is extracted to the Committee member at the end of each epoch if the Committee member is net positive.
...
Right after an election, every Committee member starts at Level 1, if they misconduct DOUGH Token Holders can decide to raise the level of penalty.

It's in the `Delegator` best interest to make sure their`Committee member` perform the actual work and otherwise ask DOUGH Token Holders to increase their penalty.
...
`Delegators` are slashed a maximum % value `Delegator_Penalty` the amount the `Committee member` is slashed. Proposed: 40%.
